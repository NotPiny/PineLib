        ...
        for (int i = 0; i < size; i++) {
            if (i % 2 == 0) {
                container.set(i, MenuItem.of(black, inventoryClickEvent -> {
                    ...
                    container.fill(MenuItem.of(black), true);
                    container.onClose(event -> {
                        event.getPlayer().sendMessage("Container closed.");
                    });
                    container.show((org.bukkit.entity.Player) inventoryClickEvent.getWhoClicked());
                }));
            } else {
